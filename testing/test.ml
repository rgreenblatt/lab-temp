...
let test_suite =
  "Test head"
  >::: [ ( "test1"
         >:: fun _ -> assert_equal ~printer:string_of_int 42 (head [42]) )
       ; ( "test2"
         >:: fun _ -> assert_equal ~printer:string_of_int 42 (head [0; 1]) ) ]
...
let test_head =
  ...
  >::: [ ("empty" >:: fun _ -> assert_raises Not_found (fun _ -> head []))
       ; ( "test1"
         >:: fun _ -> assert_equal ~printer:string_of_int 42 (head [42]) )
       ; ( "test2"
         >:: fun _ -> assert_equal ~printer:string_of_int 0 (head [0; 1]) ) ]
...
let suite =
  "Test head"
  >::: [ ( "empty"
         >:: fun _ -> assert_raises (Failure "not found") (fun _ -> head []) )
       ; ( "test1"
         >:: fun _ -> assert_equal ~printer:string_of_int 42 (head [42]) )
       ; ( "test2"
         >:: fun _ -> assert_equal ~printer:string_of_int 0 (head [0; 1]) ) ]
...
let rec index_of : 'a -> 'a list -> int =
 fun x ys ->
  ...
  | [] ->
      raise Not_found
  | y :: ys ->
      if x = y then 0 else 1 + index_of x ys

let test_index_of =
  "index_of"
  >::: [ ( "single"
         >:: fun _ -> assert_equal ~printer:string_of_int 0 (index_of 2 [2]) )
       ; ( "many"
         >:: fun _ ->
         assert_equal ~printer:string_of_int 2 (index_of 7 [1; 2; 7; 12]) )
       ; ( "many_first"
         >:: fun _ ->
         assert_equal ~printer:string_of_int 0 (index_of 1 [1; 2; 7; 12]) )
       ; ( "many_last"
         >:: fun _ ->
         assert_equal ~printer:string_of_int 3 (index_of 12 [1; 2; 7; 12]) )
       ; ("empty" >:: fun _ -> assert_raises Not_found (fun _ -> index_of 0 []))
       ; ( "single_not_found"
         >:: fun _ -> assert_raises Not_found (fun _ -> index_of 3 [2; 8]) )
       ; ( "few_not_found"
         >:: fun _ -> assert_raises Not_found (fun _ -> index_of 4 [1; 8]) )
       ; ( "many_not_found"
         >:: fun _ -> assert_raises Not_found (fun _ -> index_of 4 [2; 5; 1; 8])
         ) ]

let () =
  run_test_tt_main
    ("test" >::: [(*suite; test_suite;*) test_head; test_index_of])
